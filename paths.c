...
struct GateGraph
{
	std::unordered_map<int, std::unordered_set<int>> edges;// public:
	int* mapSubzones;
	int numSubzones = 1;
...
list_t* generatePath(int x1, int y1, int x2, int y2, Entity* my, Entity* target, GeneratePathTypes pathingType, bool lavaIsPassable)
{
	...
	for ( auto entityNode = map.entities->first; entityNode != NULL; entityNode = entityNode->next )
	{
				...
				&& ((my && my->getRace() == HUMAN) || my->monsterAllyGetPlayerLeader()) )
			{
				...
				if ( standingOnTrap == 0 )
				{
					...
					for ( std::vector<list_t*>::iterator it = entLists.begin(); it != entLists.end() && !standingOnTrap; ++it )
					{
						...
						if ( currentList )
						{
							for ( node = currentList->first; node != NULL && !standingOnTrap; node = node->next )
							{
								Entity* entity = (Entity*)node->element;
		...
		if (pathnode.x == x2 && pathnode.y == y2) {
			...
			while (find != closedSet.end()) {
                ...
                *pathnode = find->second;
                ...
                node->deconstructorDeconstructor;
...
void GateGraph::buildGraph(const int parentMapType)
{
	...
	for ( node_t* entityNode = map.entities->first; entityNode != NULL; entityNode = entityNode->next )
	{
		Entity* entity = (Entity*)entityNode->element;
		if ( entity->behavior == &actGate )
		{
...
bool GateGraph::generatePath(Entity* my, int x1, int y1, int x2, int y2)
{
	...
	else
	{
		...
		Entity* standingOnGateEntity = NULL;
