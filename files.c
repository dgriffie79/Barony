...
static const float hellAmbience = 32.f;
...
int loadMap(const char* filename2, map_t* destmap, list_t* entlist, list_t* creatureList, int *checkMapHash)
{
	...
	if ( destmap == &map )
	{
		...
		{
			if ( cameras[i].vismap != NULL )
			{
				free(cameras[i].vismap);
				cameras[i].vismap = NULL;
			}
		}
	}
    ...
    static const int numTileAtlases = sizeof(AnimatedTile::indices) / sizeof(AnimatedTile::indices[0]);
    for (int c = 0; c < mapsize; ++c) {
	...
	for (c = 0; c < numentities; c++)
	{
		...
		entity = newEntity(sprite, 0, entlist, NULL); //TODO: Figure out when we need to assign an entity to the global monster list. And do it!
		if ( editorVersion == 1 )
		{
			...
			case 6:
			case 7:
				setSpriteAttributes(entity, NULL, NULL);
				break;
			default:
				break;
			}
...
void generatePolyModels(int start, int end, bool forceCacheRebuild)
{
    ...
    static const auto LARGEST_POLYMODEL_FACES_ALLOWED = (1<<17); // 131072
...
#ifdef VERTEX_ARRAYS_ENABLED
		GL_CHECK_ERR(glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, NULL));
...
#ifdef VERTEX_ARRAYS_ENABLED
		GL_CHECK_ERR(glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, NULL));
...
#ifdef VERTEX_ARRAYS_ENABLED
		GL_CHECK_ERR(glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 0, NULL));
	...
	for ( int c = 0; !fp->eof(); c++ )
	{
		...
		if ( PHYSFS_getRealDir(name) != NULL )
		{
			...
			if ( soundHasBeenModified || reloadAll || soundRealDir.compare("./") != 0 )
			{
				...
				fmod_result = fmod_system->createSound(soundFile.c_str(), flags, NULL, &sounds[c]); //TODO: FMOD_SOFTWARE -> FMOD_DEFAULT?
				if ( FMODErrorCheck() )
				{
	...
	if (!PHYSFS_getRealDir("images/animated.txt")) {
        ...
        if (!fp) {
            ...
            for (int c = 0; !fp->eof(); ++c) {
                ...
                static const int numIndices = sizeof(animation.indices) / sizeof(animation.indices[0]);
                char *str = line, *end;
                int index = 0;
                do {
...
{
	if (mode != FileMode::WRITE || NULL == src)
	{
		return 0U;
	}
...
{
	if (mode != FileMode::READ || NULL == buffer)
	{
		return 0U;
	}
	return 1U; //Input validation passed.
