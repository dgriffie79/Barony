	...
	if ( getPlayerControlType() == playerControlType_t::PLAYER_CONTROLLED_BY_CONTROLLER )
	{
		...
		for (auto& binding : getGamepadBindings()) {
			if ( binding.second == MainMenu::hiddenBinding )
			{
				...
				{
					// hidden binding, don't  existing bind by the defaults.
					continue;
				}
			}
	...
	if (len >= 3 && strncmp(input, "Pad", 3) == 0) {
		...
		if (find != list.end()) {
			...
			if (strncmp(type, "Button", 6) == 0) {
				...
				(*b).second.joystickButton = (Uint32)strtol((const char*)(type + 6), NULL, 10);
				return;
			} else if (strncmp(type, "Axis-", 5) == 0) {
				(*b).second.type = binding_t::JOYSTICK_AXIS;
				(*b).second.joystickAxisNegative = true;
				(*b).second.joystickAxis = (Uint32)strtol((const char*)(type + 5), NULL, 10);
				return;
			} else if (strncmp(type, "Axis+", 5) == 0) {
				(*b).second.type = binding_t::JOYSTICK_AXIS;
				...
				(*b).second.joystickHat = (Uint32)strtol((const char*)(type + 3), NULL, 10);
				if (type[3]) {
		...
		Uint32 index = (Uint32)strtol((const char*)(input + 5), NULL, 10);
