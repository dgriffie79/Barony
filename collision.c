...
bool entityInsideSomething(Entity* entity)
{
	...
	for ( std::vector<list_t*>::iterator it = entLists.begin(); it != entLists.end(); ++it )
	{
		...
		for ( node_t* node = currentList->first; node != NULL; node = node->next )
		{
				...
				if ( (hitAngle >= 0 && hitAngle <= 2 * PI / 3) ) // 120 degree arc
				{
					...
					else
					{
						if ( (monsterState == MONSTER_STATE_WAIT
							...
							|| (monsterState == MONSTER_STATE_HUNT /*&& uidToEntity(monsterTarget) == NULL*/))
									...
									if ( myStats && yourStats )
									{
										if ( myStats->getEffectActive(EFF_FLAME_CLOAK) )
										{
											hit.entity->char_fire = std::min(hit.entity->char_fire, 
												getSpellEffectDurationSecondaryFromID(SPELL_FLAME_CLOAK, my, NULL, my));
										}
									...
									if ( myStats && yourStats )
									{
										if ( yourStats->getEffectActive(EFF_FLAME_CLOAK) )
										{
											my->char_fire = std::min(my->char_fire,
												getSpellEffectDurationSecondaryFromID(SPELL_FLAME_CLOAK, hit.entity, NULL, hit.entity));
										}
...
Entity* findEntityInLine( Entity* my, real_t x1, real_t y1, real_t angle, int entities, Entity* target, list_t* entityListToUse)
{
	...
	for ( std::vector<list_t*>::iterator it = entLists.begin(); it != entLists.end(); ++it )
	{
		...
		for ( node = currentList->first; node != NULL; node = node->next )
		{
			...
			if ( (entity != target && target != NULL) || entity->flags[PASSABLE] || entity == my
				...
				)
			{
				...
				{
					if ( (entity != target && target != NULL) || entity == my || entity->flags[PASSABLE] )
					{
						continue;
					}
