...
Stat* Stat::copyStats()
{
	...
	newStat->void_chest_inventory.first = NULL;
	newStat->void_chest_inventory.last = NULL;
...
int Stat::pickRandomEquippedItem(Item** returnItem, bool excludeWeapon, bool excludeShield, bool excludeArmor, bool excludeJewelry)
{
	...
	if ( !excludeArmor )
	{
		if ( this->helmet != NULL && this->helmet->status > BROKEN )
		{
			equipNum[numEquippedItems] = 0;
			++numEquippedItems;
		}
		if ( this->breastplate != NULL && this->breastplate->status > BROKEN )
		{
			equipNum[numEquippedItems] = 1;
			++numEquippedItems;
		}
		if ( this->gloves != NULL && this->gloves->status > BROKEN )
		{
			equipNum[numEquippedItems] = 2;
			++numEquippedItems;
		}
		if ( this->shoes != NULL && this->shoes->status > BROKEN )
		{
			equipNum[numEquippedItems] = 3;
			++numEquippedItems;
		}
		if ( this->cloak != NULL && this->cloak->status > BROKEN )
		{
			equipNum[numEquippedItems] = 6;
			++numEquippedItems;
		}
		if ( this->mask != NULL && this->mask->status > BROKEN )
		{
	...
	if ( !excludeWeapon )
	{
		if ( this->weapon != NULL && this->weapon->status > BROKEN )
		{
	...
	if ( !excludeShield )
	{
		if ( this->shield != NULL && this->shield->status > BROKEN )
		{
	...
	if ( !excludeJewelry )
	{
		if ( this->amulet != NULL  && this->amulet->status > BROKEN )
		{
			equipNum[numEquippedItems] = 7;
			++numEquippedItems;
		}
		if ( this->ring != NULL && this->ring->status > BROKEN )
		{
...
void Stat::copyNPCStatsAndInventoryFrom(Stat& src)
{
	...
	if ( src.helmet )
	{
		if ( player >= 0 )
		{
			Item* item = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
		...
		else
		{
			if ( !this->helmet )
			{
				this->helmet = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.breastplate )
	{
		...
		else
		{
			if ( !this->breastplate )
			{
				this->breastplate = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.gloves )
	{
		...
		else
		{
			if ( !this->gloves )
			{
				this->gloves = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.shoes )
	{
		...
		else
		{
			if ( !this->shoes )
			{
				this->shoes = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.shield )
	{
		...
		else
		{
			if ( !this->shield )
			{
				this->shield = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.weapon )
	{
		...
		else
		{
			if ( !this->weapon )
			{
				this->weapon = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.cloak )
	{
		...
		else
		{
			if ( !this->cloak )
			{
				this->cloak = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.amulet )
	{
		...
		else
		{
			if ( !this->amulet )
			{
				this->amulet = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.ring )
	{
		...
		else
		{
			if ( !this->ring )
			{
				this->ring = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
	...
	if ( src.mask )
	{
		...
		else
		{
			if ( !this->mask )
			{
				this->mask = newItem(GEM_ROCK, EXCELLENT, 0, 1, 0, true, NULL);
			}
...
void Stat::addItemToLootingBag(const int player, const real_t x, const real_t y, Item& item)
{
	...
	{
		item.applyDuck(achievementObserver.playerUids[player], x, y, NULL, false);
		return;
	}
	...
	if ( player_lootbags.find(lootingBagKey) == player_lootbags.end() )
	{
		...
		{
			Entity* entity = newEntity(-1, 1, map.entities, NULL); //Item entity.
			...
			if ( !stats[i]->player_lootbags[key].looted )
			{
				for ( auto& item_loot : stats[i]->player_lootbags[key].items )
				{
					...
					Item* item2 = newItem(item_loot.type, item_loot.status, 
						item_loot.beatitude, item_loot.count, item_loot.appearance, item_loot.identified, NULL);
					if ( item2 )
					{
